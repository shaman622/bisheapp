...
const DataPreprocessing: React.FC = () => {
  ...
  const [adjacencyFileName, setAdjacencyFileName] = useState<string>('未选择文件');
  ...
  // 处理数据集邻接矩阵文件上传
  const handleAdjacencyUpload = () => {
    if (selectedAdjacencyFile) {
      console.log('上传数据集邻接矩阵文件:', selectedAdjacencyFile.name);
      alert(`数据集邻接矩阵文件 "${selectedAdjacencyFile.name}" 上传成功！`);
    } else {
      alert('请先选择一个数据集邻接矩阵文件！');
    }
  };
    ...
    <div className="section-card">
      ...
      <div className="file-upload-row">
        ...
        <div className="file-upload-container">
          ...
          <button onClick={handleFeatureUpload} className="upload-button">
            上传
          </button>
        </div>
      </div>
      ...
      <div className="file-upload-row">
        <h3>数据集邻接矩阵上传</h3>
        <div className="file-upload-container">
          <label htmlFor="adjacency-file-input" className="file-label">
            选择文件
          </label>
          <input
            id="adjacency-file-input"
            type="file"
            onChange={handleAdjacencyFileChange}
            style={{ display: 'none' }}
          />
          <span className="file-name">{adjacencyFileName}</span>
          <button onClick={handleAdjacencyUpload} className="upload-button">
            上传
          </button>
        </div>
      </div>
